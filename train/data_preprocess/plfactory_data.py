...
PROMPT_TEMPLATE_POSTGRES = """You are an expert in postgres database and PL/SQL programming. Please generate a PL/pgSQL following the natural language instruction.

Instruction:
{ir_text}

Please generate the PL/pgSQL code based on the instruction."""


PROMPT_TEMPLATE_ORACLE = """You are an expert in oracle database and PL/SQL programming. Please generate an Oracle PL/SQL following the natural language instruction.

Instruction:
{ir_text}

Please generate the Oracle PL/SQL code based on the instruction."""
...
def main():
    ...
    parser.add_argument("--spider_postgres_jsons", nargs=1, required=True, help="Spider Postgres JSON 文件路径")
    parser.add_argument("--omni_postgres_jsons", nargs=1, required=True, help="Omni Postgres JSON 文件路径")
    parser.add_argument("--spider_oracle_jsons", nargs=1, required=True, help="Spider Oracle JSON 文件路径")
    parser.add_argument("--omni_oracle_jsons", nargs=1, required=True, help="Omni Oracle JSON 文件路径")
    parser.add_argument("--local_save_dir", default="/workspace/opt/projects/verlpl/examples/results")
    parser.add_argument("--hdfs_dir", default=None)
    parser.add_argument("--train_output", default="plfactory_rl_train_new_2.parquet")
    parser.add_argument("--test_output", default="plfactory_rl_test_new_2.parquet")
    parser.add_argument("--train_per_spider_file", type=int, default=1050)
    parser.add_argument("--train_per_omni_file", type=int, default=450)
    parser.add_argument("--test_per_spider_file", type=int, default=210)
    parser.add_argument("--test_per_omni_file", type=int, default=90)
    ...
    # --- 1) 构建 file_specs：显式标注来源 source=spider/omni，并保留各自配额 ---
    file_specs: List[Dict[str, Any]] = []

    def add_specs(paths: List[str], backend: str, source: str, label_prefix: str):
        for idx, path in enumerate(paths, start=1):
            groups = load_grouped_json(path)
            file_specs.append(
                {
                    "path": os.path.expanduser(path),
                    "backend": backend,          # postgres/oracle
                    "source": source,            # spider/omni
                    "label": f"{label_prefix}_{idx}",
                    "groups": groups,
                }
            )

    add_specs(args.spider_postgres_jsons, backend="postgres", source="spider", label_prefix="spider_postgres")
    add_specs(args.omni_postgres_jsons,   backend="postgres", source="omni",   label_prefix="omni_postgres")
    add_specs(args.spider_oracle_jsons,   backend="oracle",   source="spider", label_prefix="spider_oracle")
    add_specs(args.omni_oracle_jsons,     backend="oracle",   source="omni",   label_prefix="omni_oracle")

    # --- 2) 分别抽样、分别计数，最终合并写 parquet ---
    train_records_all: List[Dict[str, Any]] = []
    test_records_all: List[Dict[str, Any]] = []

    # 统一成 allocate_extra_samples 需要的结构：backend -> List[{label, records}]
    leftovers_by_backend: Dict[str, List[Dict[str, Any]]] = {"postgres": [], "oracle": []}

    # 统计用：按 backend + source 计数
    counters = {
        "postgres": {"spider": {"train": 0, "test": 0}, "omni": {"train": 0, "test": 0}},
        "oracle":   {"spider": {"train": 0, "test": 0}, "omni": {"train": 0, "test": 0}},
    }

    def per_file_quota(source: str) -> Tuple[int, int]:
        if source == "spider":
            return args.train_per_spider_file, args.test_per_spider_file
        if source == "omni":
            return args.train_per_omni_file, args.test_per_omni_file
        raise ValueError(f"未知 source: {source}")

    for spec in file_specs:
        annotate_records_with_type(spec["groups"], spec["backend"])

        train_size, test_size = per_file_quota(spec["source"])
        ...
        counters[spec["backend"]][spec["source"]]["train"] += len(selections["train"])
        counters[spec["backend"]][spec["source"]]["test"] += len(selections["test"])

        print(
            f"[INFO] {spec['label']} ({spec['backend']}, {spec['source']}) -> "
            f"train {len(selections['train'])}, test {len(selections['test'])}"
        )

        rng.shuffle(leftovers)
        leftovers_by_backend[spec["backend"]].append(
            {"label": spec["label"], "records": leftovers}
        )

    # --- 3) 校验预期数量（按 spider/omni 分开累计后再合并）---
    spider_file_cnt = (
        len(args.spider_postgres_jsons) + len(args.spider_oracle_jsons)
    )
    omni_file_cnt = (
        len(args.omni_postgres_jsons) + len(args.omni_oracle_jsons)
    )

    expected_train = args.train_per_spider_file * spider_file_cnt + args.train_per_omni_file * omni_file_cnt
    expected_test = args.test_per_spider_file * spider_file_cnt + args.test_per_omni_file * omni_file_cnt
    ...
    print("[INFO] 采样统计（按 backend/source 分开计数）:")
    for backend in ("postgres", "oracle"):
        for source in ("spider", "omni"):
            t = counters[backend][source]["train"]
            v = counters[backend][source]["test"]
            print(f"       - {backend}/{source}: train={t}, test={v}")

    # --- 4) 组装 HF dataset 并写 parquet（仍是合并后的总 train/test）---
