...
from config.common import pg_config, strong_llm_config, seed_generation_config
...
def _call_critical_llm_with_retry(prompt, max_retries: int = 3, timeout: float = 120.0):
    ...
    Returns:
        LLM响应对象
    """
    critical_model_cfg = strong_llm_config.get("critical_model", {})
...
def generate_call_sql(state: SeedGenerationState, plsql_code):
...
def critical(state: SeedGenerationState):
    dialect = state["dialect"]
    print(state["selected_database_name"])
    ...
    if 2 <= critical_epoch <= 2:
        if not any(state['need_correction']):
            state["seeds"] = []
            for i, plsql in enumerate(state["generated_plsqls"]):
                state["seeds"].append({"plsql": plsql, 
                                       "database_name": state["selected_database_name"], 
                                       "tables": state["selected_tables"]})
            state["critical_epoch"] = 3
            return state
    ...
    for i, plsql_code in enumerate(state["generated_plsqls"]):
    ...
    # garbage collect and save seeds
    state["seeds"] = []
    garbages = []  
    for i, plsql in enumerate(state["generated_plsqls"]):
