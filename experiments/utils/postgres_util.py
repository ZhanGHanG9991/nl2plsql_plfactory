...
from experiments.settings.expr_setting import pg_config, get_dataset_config
...
def set_db_connection_info(new_host=None, new_port=None):
    """
    设置数据库连接信息
    ...
    Args:
        ...
        new_port: 新的端口号
    """
    global host, port, conn_info
    if new_host:
        host = new_host
    if new_port:
        port = new_port
    
    # Update conn_info with new values
    conn_info = f"host={host} user={user} password={password} dbname={pg_config['dbname']} port={port}"
    print(f"Updated DB connection info: host={host}, port={port}")

def get_tables_info(database_name):
    conn_db_info = f"""host={host} port={port} dbname={database_name} user={user} password={password}"""
    tables_info = {}
...
def get_table_column_set(database_name: str, schema_dict: Dict = None) -> Set[str]:
    """
    获取指定数据库的所有表名和列名的集合（保留原始大小写）
    
    Args:
        database_name: 数据库名称
        schema_dict: schema字典，如果为None则自动加载
    
    Returns:
        Set[str]: 包含所有表名和列名的集合（保留原始大小写）
    """
    ...
    if database_name not in schema_dict:
        print(f"Warning: database '{database_name}' not found in schema_dict")
        return identifiers
    
    db_info = schema_dict[database_name]
...
def quote_identifiers_in_plsql(plsql: str, identifiers: Set[str]) -> str:
    """
    在 PL/SQL 代码中给表名和列名加上双引号
    
    Args:
        plsql: 原始 PL/SQL 代码
        identifiers: 需要加引号的标识符集合（保留原始大小写）
    
    Returns:
        处理后的 PL/SQL 代码
    """
    if not plsql or not identifiers:
        return plsql
    ...
    while i < n:
        ...
        if i + 1 < n and plsql[i:i+2] == '/*':
            end_idx = _find_block_comment_end_for_quote(plsql, i)
        ...
        if i + 1 < n and plsql[i:i+2] == '--':
            end_idx = plsql.find('\n', i)
            if end_idx != -1:
                result.append(plsql[i:end_idx+1])
                i = end_idx + 1
            else:
        ...
        # 3. 处理 Dollar-quoted 标记 ($$, $tag$)：只保留标记本身，继续处理内部内容
        if plsql[i] == '$':
            # 匹配 $$ 或 $tag$
            j = i + 1
            while j < n and (plsql[j].isalnum() or plsql[j] == '_'):
                j += 1
            if j < n and plsql[j] == '$':
                # 找到了 $...$ 标记，只添加这个标记
                tag = plsql[i:j+1]
        ...
        # 4. 跳过单引号字符串
        if plsql[i] == "'":
            end_idx = _find_string_end_for_quote(plsql, i, "'")
            result.append(plsql[i:end_idx])
            i = end_idx
            continue
        ...
        # 6. 检查是否是标识符的开始
        if plsql[i].isalpha() or plsql[i] == '_':
            # 提取完整的标识符
            j = i
            while j < n and (plsql[j].isalnum() or plsql[j] == '_'):
                j += 1
            
            identifier = plsql[i:j]
            
            # 检查是否需要加引号（完全匹配，区分大小写）
        ...
        # 7. 处理点号（可能是 table.column 格式）
        if plsql[i] == '.':
            # 检查点号前后是否有引号包裹的标识符
            # 如果前面刚添加了带引号的标识符，点号后面的标识符也需要处理
            result.append('.')
            i += 1
            continue
...
def _find_block_comment_end_for_quote(s: str, start: int) -> int:
    """找到块注释的结束位置，支持嵌套"""
    n = len(s)
    i = start + 2
    depth = 1
...
def _find_string_end_for_quote(s: str, start: int, quote_char: str) -> int:
    """找到字符串的结束位置"""
    n = len(s)
    i = start + 1
...
def quote_plsql_identifiers(plsql: str, database_name: str, schema_dict: Dict = None) -> str:
    """
    便捷函数：给 PL/SQL 代码中的表名和列名加上双引号
    ...
    Returns:
        处理后的 PL/SQL 代码
    """
    identifiers = get_table_column_set(database_name, schema_dict)
    return quote_identifiers_in_plsql(plsql, identifiers)
...
def compare_plsql(database_name, plsql1, plsql2, call_plsqls, include_system_tables):
    ...
    """
    try:
        database_conn_info = f"""host={host} port={port} user={user} password={password} dbname={database_name}"""
