...
from experiments.utils import oracle_util, plsql_util
...
def extract_info_from_filename(filename):
    ...
    文件名格式示例:
    - oracle_spider_function_test-deepseek-0_shot
    - oracle_omni_procedure_test-gpt4-3_shot
    - oracle_plforge_trigger_test-claude
    ...
    Returns:
        tuple: (数据集名称, 数据库名称, 类型)
            - 数据集名称: oracle_spider_function_test (用于 initialize_dataset_paths)
    ...
    # 例如: oracle_spider_function_test
    if len(parts) >= 4:
...
def evaluate_oracle_json(file_path, host=None, port=None):
    """
    读取JSON文件并评测 Oracle PL/SQL 代码的准确性。
    ...
    # 设置数据库连接信息
    if host or port:
        oracle_util.set_db_connection_info(host, port)
    ...
    print(f"开始评测 Oracle 数据，共 {total_count} 条记录...")
    print("=" * 60)
    ...
    for idx, item in enumerate(data, 1):
        ...
        processed_predicted_plsql = plsql_util.post_process_plsql(raw_predicted_plsql)
        ...
        # EM 评测：使用 AST 语义匹配
        is_em_match = oracle_util.is_exact_match(processed_gold_plsql, processed_predicted_plsql)
        if is_em_match:
            em_score += 1
            print(f"  EM: ✓ (语义匹配)")
        else:
        ...
        try:
            ...
            if plsql_type == 'function':
                ...
                is_ex_match = oracle_util.compare_plsql_function(
                    database_name, 
                    gold_plsql, 
                    raw_predicted_plsql, 
                    call_sqls
                ...
                is_ex_match = oracle_util.compare_plsql(
                    database_name, 
                    gold_plsql, 
                    raw_predicted_plsql, 
                    call_sqls,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="评测 Oracle PL/SQL 代码准确性")
    ...
    evaluate_oracle_json(file_path, args.host, args.port)