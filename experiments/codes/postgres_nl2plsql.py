...
from experiments.utils import postgres_util

# ==========================================
# 1. PostgreSQL特定的Prompt格式化函数
# ==========================================
def format_prompt_for_local_model(databases_schema_info: str, ir_text: str) -> str:
    ...
    """
    prompt = (
            f"You are an expert in PostgreSQL database and PL/pgSQL programming. Please generate a PostgreSQL PL/pgSQL code "
            f"based on the provided database schema information and following the natural language instruction.\n\n"
            f"Schema Info:\n{databases_schema_info}\n\n"
            f"Instruction:\n{ir_text}\n\n"
            f"Please generate the PostgreSQL PL/pgSQL code based on the schema and instruction.\n"
            f"IMPORTANT: You MUST wrap your final PL/pgSQL code strictly within <start-plsql> and <end-plsql> tags. Do not generate any comments or any other extraneous information; output only PL/pgSQL.\n"
            f"Your response:"
        )
...
# ==========================================
POSTGRES_ZERO_SHOT_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are an expert in PostgreSQL database and PL/pgSQL programming. "
            "Your task is to generate valid PL/pgSQL code based on the provided Intermediate Representation (IR) and Database Schema."
        ),
        (
            "user",
            (
                ...
                "Please generate the corresponding PL/pgSQL code.\n"
                "IMPORTANT: You MUST wrap your final PL/pgSQL code strictly within <start-plsql> and <end-plsql> tags. Do not generate any comments or any other extraneous information; output only PL/pgSQL.\n"
                "Your response:"
            )
        ),
    ]
...
def run_postgres_zero_shot_experiment(dataset_name: str, model_name: str, max_tokens: int = 8192, input_key: str = "ir"):
    """
    运行 PostgreSQL Zero-Shot 实验
    ...
    """
    print("=" * 60)
    print("PostgreSQL Zero-Shot NL2PL/SQL 实验")
    print("=" * 60)
    ...
    for i, seed in enumerate(seeds):
        ...
        # 构建 Schema 信息
        try:
            detailed_database_schema = postgres_db_schema_dict[database_name]
            table_schemas_str = postgres_util.generate_schema_prompt_from_dict(
        ...
        try:
            ...
            prompt_messages = POSTGRES_ZERO_SHOT_PROMPT.format_messages(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PostgreSQL Zero-Shot NL2PL/SQL 实验",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 使用API模型
  python postgres_nl2plsql_zero_shot.py --dataset postgres_spider_function_test --model gpt-4o
  
  # 使用本地模型
  python postgres_nl2plsql_zero_shot.py --dataset postgres_spider_function_test --model /home/zhanghang/opt/projects/researchprojects/text2PLSQL/models/starcoder2-3b
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="数据集名称 (如: postgres_spider_function_test)"
    )
    ...
    run_postgres_zero_shot_experiment(
        args.dataset, 
        args.model, 
